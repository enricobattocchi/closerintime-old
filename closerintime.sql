...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `verification_view` AS select `suggestions`.`uuid` AS `uuid`,'SUGGESTION' AS `source_uuid` from `suggestions` union select `substitutions`.`target_uuid` AS `uuid`,`substitutions`.`source_uuid` AS `source_uuid` from `substitutions`;
